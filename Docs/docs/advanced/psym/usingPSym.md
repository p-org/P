...
the run. Assuming uniform probability for each scheduling/data choice, this metric reports the probability of a 
...
??? tip "Coverage Report"
    ...
    At the end of a run, PSym prints a coverage report in `` output/coverage-*.log `` that tabulates, for each 
    ...
    ??? example "Coverage Report: `` output/coverage-clientserver.log ``"
    ...
    Note that PSym's coverage report can be incomplete, since it tabulates the remaining choices as of by the end of the run.
    Each remaining choice, say at a step N, when explored can discover many more choices at steps >= N+1.
...
??? tip "Continuous Feedback"
    ...
    Want more feedback? Run with `` --stats 2 `` to print coverage after each scheduling step.
