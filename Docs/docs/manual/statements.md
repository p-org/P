A Function (i.e., entry, exit, or event handlers) in P can be arbitrary piece of imperative code and 
hence allows programmers to capture complex protocol logic in their state machines. 
P supports common imperative programming language constructs like while-loops, function calls, and conditionals.
