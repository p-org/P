...
def find_mean_event_latency(logs):
    ...
    return avg_t, p99_t, int(n_requests), int(n_responses), n_responses/(request_qps*(max_time - 1) + request_qps*k_client_amplification)*100, request_qps, response_qps, max_time, n_server_fails, k_client_amplification, n_retries, retry_rate_threshold
