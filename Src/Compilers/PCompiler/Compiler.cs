namespace DemoCompiler
{
    ...
    internal class Compiler
    {
        ...
        private MyDictionary<string, AST<FuncTerm>> modelAliases = 
            new MyDictionary<string, AST<FuncTerm>>();
        ...
        Dictionary<Node, TypeCheckInfo> computedType =
            new Dictionary<Node, TypeCheckInfo>();
        ...
        private void EraseGhostDecls()
        {
            ...
            foreach (string s in declNames2)
            {
                ...
                foreach (var e in bin)
                {
                    var stateDecl = modelAliases[((Id)GetArgByIndex(e.Node, 0)).Name];
                    var ownerName = GetOwnerName(stateDecl.Node, 1, 0);
                    if (allMachines[ownerName].isGhost)
                        continue;
                    newDecls.AddLast(e);
                }
...
--";
        ...
        private AST<FuncTerm> MkTypeDecl(PType t)
        {
            ...
            return MkInit(MkStringLiteral(cTypeName), GetCTypeSize(t), cPrimitive, def, MkId(pTypeToCEnum(t) + "SuperTypes"), MkId(pTypeToCEnum(t) + "SubTypes"), clone, buildDef, destroy);
        }
        ...
        private AST<Node> MkEq(AST<Node> e1, PType t1, AST<Node> e2, PType t2)
        {
            PType supT, subT;
            AST<Node> supE, subE;
            ...
            if (t1 == t2)
            {
                ...
                {
                    return MkFunApp(getCEqualsName(t1, t2), default(Span), MkAddrOf(e1), MkAddrOf(e2));
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    return MkFunApp(getCEqualsName(supT, subT), default(Span), MkAddrOf(supE), subArg);
                }
            }
        }
        ...
        private AST<Node> MkCanCast(AST<Node> from, PType fromT, PType toT)
        {
            ...
            else if (toT.isSubtypeOf(fromT))    // toT <: FromT
            {
                ...
                {
                    return MkEq(from, fromT, null, toT); // Its safe to pass null here since the expression is ignore when comparing with Null
                }
            ...
            foreach (var t in allTypes)
            {
                ...
                foreach (var subT in subtypes[t])
                {
                    if (t is PAnyType)
                    {
                        ...
                        {
                            cases.Add(new Tuple<AST<Node>, AST<Node>>(MkId(pTypeToCEnum(midT)),
                                AddArgs(CData.App_Return(), MkCanCast(getPackedValueMember(MkId("obj"), midT), midT, subT))));
                        }
                    ...
                    else if (t is PTupleType || t is PNamedTupleType)
                    {
                        ...
                        for (int i = 0; i < fromFields.Count(); i++)
                        {
                            var canCast = MkCanCast(MkArrow(MkId("obj"), fromFields[i].Item3), fromFields[i].Item1, toFields[i].Item1);
        ...
        private void MkEqualsMethods(List<AST<Node>> forwardDecls, List<AST<Node>> methodDefs)
        {
            foreach (var t in allTypes)
            {
                ...
                foreach (var subT in subtypesAndMe(t))
                {
                    ...
                    var funType = MkFunType(pTypeToCType(PType.Bool), arg1T, arg2T);
                    var prms = MkFunParams(false, "e1", "e2");
                    var body = new List<AST<Node>>();
                    ...
                    if (t is PAnyType)
                    {
                        ...
                        {   // Switch on e1's type, and in each case defer to comparing an "Any" value with a concrete type.
                            ...
                            foreach (var rightT in subtypes[t])
                            {
                                cases.Add(new Tuple<AST<Node>, AST<Node>>(MkId(pTypeToCEnum(rightT)),
                                        AddArgs(CData.App_Return(), MkEq(MkDrf(MkId("e1")), t, getPackedValueMember(MkId("e2"), rightT), rightT))));
                            }
                        ...
                        {   // Comparing an Any type to a concrete type T. Check if Any holds any relative of T, or T itself, and if so, invoke
                            ...
                            foreach (var midT in relatives(subT).Where(tp => !(tp is PAnyType)))
                            {
                                ...
                                cases.Add(new Tuple<AST<Node>, AST<Node>>(MkId(pTypeToCEnum(midT)),
                                    AddArgs(CData.App_Return(), MkEq(getPackedValueMember(MkId("e1"), midT), midT, arg2, subT))));
                            }
                    ...
                    else if (t is PTupleType || t is PNamedTupleType)
                    {
                        ...
                        var fieldsEqualityTerms = e1Fields.Zip(e2Fields, (supField, subField) =>
                            MkEq(MkArrow("e1", supField.Item3), supField.Item1, MkArrow("e2", subField.Item3), subField.Item1)).ToArray<AST<Node>>();
    ...
    it also contains tables for event sets, transitions, actions, states, and variables.",
        ...
        private AST<Node> MkStateTable(AST<FuncTerm> machine)
        {
            ...
            foreach (var s in bin)
            {
                ...
                var defersSet = (Id)GetArgByIndex(s.Node, 3);
                var data = MkInit(
                    ...
                    defersSet.Name == "NIL" ? MkId(string.Format("EventSet_{0}_None", machName)) : MkId(string.Format("EventSet_{0}_{1}", machName, GetName((FuncTerm)modelAliases[defersSet.Name].Node, 0))),
        ...
        private void MkExitFuns(out AST<FuncTerm> headers, out AST<FuncTerm> bodies)
        {
            ...
            foreach (var s in bin)
            {
                ...
                {
                    it.MoveNext();
                    var stateDecl = modelAliases[((Id)it.Current).Name].Node;
        ...
        {
            Id id = (Id)node;
            if (id.Name != PData.Cnst_Default.Node.Name)
            {
            ...
            int ownerNameIndex)
        {
            ...
            {
                case NodeKind.Id:
                    return ((Cnst)GetArgByIndex(modelAliases[((Id)ownerArg).Name].Node, ownerNameIndex)).GetStringValue();
            ...
            int ownerOwnerNameIndex)
        {
            ...
            {
                case NodeKind.Id:
                    return GetOwnerName(modelAliases[((Id)ownerArg).Name].Node, ownerOwnerIndex, ownerOwnerNameIndex);
                case NodeKind.FuncTerm:
        ...
        private AST<Node> EntryFun_Fold_Impl(PToCFoldContext ctxt, Node n, IEnumerable<AST<Node>> children)
        {
            ...
            else if (funName == PData.Con_Apply.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    else if (arity == 2)
                    {
                        ...
                        {
                            var app = MkEq(arg1, arg1Type, arg2, arg2Type);
            ...
            else if (funName == PData.Con_Raise.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    ctxt.addSideEffect(MkAssert(MkFunApp(getCCanCastName(PType.Any), default(Span), args[2], eventPayloadType),
            ...
            else if (funName == PData.Con_Send.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    // Emit an assert that the payload can be cast to the event's expected type.
                    var eventPayloadType = MkDot(MkIdx(MkArrow(ctxt.driver, "Events"), args[1]), "Type");
                    ctxt.addSideEffect(MkAssert(MkFunApp(getCCanCastName(PType.Any), default(Span), args[2], eventPayloadType), n.Span, "Payload not Cast-able to expected event payload on Send"));
            ...
            else if (funName == PData.Con_While.Node.Name)
            {
                ...
                outTerm = Factory.Instance.AddArg(CData.App_Loop(n.Span), CData.Cnst_While(n.Span));
                using (var it = children.GetEnumerator())
                {
                    it.MoveNext();
                    outTerm = Factory.Instance.AddArg(outTerm, ctxt.consumeExp(it.Current));
                    it.MoveNext();
        ...
        private AST<Node> ConstructList(AST<FuncTerm> constructor, params AST<Node>[] elems)
        {
        ...
        private AST<FuncTerm> MkZingIdentifier(string name)
        {
            return Factory.Instance.AddArg(ZingData.App_Identifier, Factory.Instance.MkCnst(name));
        }

        private AST<Node> MkZingDot(params string[] names)
        {
        ...
        private AST<Node> MkZingIf(AST<Node> cond, AST<Node> then)
        {
        ...
        private AST<FuncTerm> MkZingApply(AST<Id> op, params AST<Node>[] args)
        {
        ...
        private AST<Node> GenerateCalculateDeferredAndActionSetMethodDecl(FuncTerm stateDecl)
        {
            ...
            var deferredSetDecl = (Id)GetArgByIndex(stateDecl, 3);
            var ownerName = GetOwnerName(stateDecl, 1, 0);

            List<string> localDeferredSet = new List<string>();
            if (deferredSetDecl.Name != PData.Cnst_Nil.Node.Name)
            {
                string eventSetName, machineName;
                DeconstructEventSetDecl(modelAliases[deferredSetDecl.Name].Node, out eventSetName, out machineName);
        ...
        private AST<Node> GenerateActionHelperMethodDecl(string machineName)
        {
            ...
            foreach (var actionFunName in allMachines[machineName].actionFunNameToActionFun.Keys)
            {
                ...
                var executeStmt = MkZingSeq(
                                    ...
                                    GenerateActionFunction(machineName, actionFunName, allMachines[machineName].actionFunNameToActionFun[actionFunName].actionFun, locals)
                                    );
        ...
        private AST<Node> GenerateRunHelperMethodDecl(string machineName)
        {
            ...
            foreach (var stateDecl in allMachines[machineName].stateNameToStateInfo.Values.Select(x => x.stateDecl))
            {
                ...
                var executeStmt = MkZingSeq(
                                    ...
                                    GenerateEntryFunction(stateDecl, locals), 
                ...
                AST<Node> exitFunction = GenerateExitFunction(stateDecl, locals);
        ...
        class ZingEntryFun_FoldContext
        {
            ...
            public AST<Node> getTmpVar(AST<Node> type, string baseName)
            {
                ...
                var tmpVar = compiler.MkZingIdentifier(tmpVarName);
        ...
        private AST<Node> GenerateActionFunction(string machineName, string actionFunName, Node actionFun, List<AST<Node>> locals)
        {
            AST<Node> outTerm = MkZingBody(actionFun, machineName, TranslationContext.Action, actionFunName, locals);
            if (allMachines[machineName].actionFunNameToActionFun[actionFunName].argTypes.Count == 0)
            {
                // This should only happen if an action is declared but never installed
                allMachines[machineName].actionFunNameToActionFun[actionFunName].argTypes.Add(new PNilType());
            }
            
            outTerm = MkZingSeq(outTerm, AddArgs(ZingData.App_Goto, Factory.Instance.MkCnst("exit")));
            return outTerm;
        }

        private AST<Node> GenerateEntryFunction(FuncTerm stateDecl, List<AST<Node>> locals)
        {
            var stateName = GetName(stateDecl, 0);
            var machineAlias = (Id)GetArgByIndex(stateDecl, 1);
            var machineName = GetName(modelAliases[machineAlias.Name].Node, 0);
            Node entry = GetArgByIndex(stateDecl, 2);

            return MkZingBody(entry, machineName, TranslationContext.Entry, stateName, locals);
            }

        private AST<Node> GenerateExitFunction(FuncTerm stateDecl, List<AST<Node>> locals)
        {
            var stateName = GetName(stateDecl, 0);
            var machineAlias = (Id)GetArgByIndex(stateDecl, 1);
            var machineName = GetName(modelAliases[machineAlias.Name].Node, 0);
            Node exit = allMachines[machineName].stateNameToStateInfo[stateName].exitFun;

            return (exit == null ? ZingData.Cnst_Nil : MkZingBody(exit, machineName, TranslationContext.Exit, stateName, locals));
        }
        ...
        private Tuple<AST<Node>, TypeCheckInfo> ZingEntryFun_Fold_Impl(ZingEntryFun_FoldContext ctxt, Node n, IEnumerable<Tuple<AST<Node>, TypeCheckInfo>> children)
        {
            ...
            if (n.NodeKind != NodeKind.FuncTerm)
            {
                ...
                else if (n.NodeKind == NodeKind.Id)
                {
                    ...
                    {
                        return new Tuple<AST<Node>, TypeCheckInfo>(null, new TypeCheckInfo(new PBoolType(), true));
                    }
                    else if (id.Name == PData.Cnst_Nil.Node.Name)
                    {
                    ...
                    else if (id.Name == PData.Cnst_Leave.Node.Name)
                    {
                        ...
                        {
                            returnNode = Factory.Instance.AddArg(ZingData.App_Goto, Factory.Instance.MkCnst("exit"));
                        }
                        else if (ctxt.translationContext == TranslationContext.Exit)
                        {
                            returnNode = Factory.Instance.AddArg(ZingData.App_Return, ZingData.Cnst_Nil);
                        }
                        else
                        {
                            returnNode = Factory.Instance.AddArg(ZingData.App_Goto, Factory.Instance.MkCnst("wait_" + entityName));
                        }
                        return new Tuple<AST<Node>, TypeCheckInfo>(returnNode, new TypeCheckInfo(new PNilType()));
                    }
                    else if (id.Name == PData.Cnst_Delete.Node.Name)
                    {
                        ...
                        var currentEventAsgnStmt = AddArgs(ZingData.App_Assign, MkZingDot("myHandle", "currentEvent"), MkZingEvent("delete"));
                        var gotoStmt = Factory.Instance.AddArg(ZingData.App_Goto, Factory.Instance.MkCnst(string.Format("transition_{0}", entityName)));
                        return new Tuple<AST<Node>, TypeCheckInfo>(MkZingSeq(currentEventAsgnStmt, gotoStmt), new TypeCheckInfo(new PNilType()));
                    }
            ...
            else if (funName == PData.Con_Return.Node.Name)
            {
                ...
                if (ctxt.translationContext == TranslationContext.Entry || ctxt.translationContext == TranslationContext.Action)
                {
                    using (var it = children.GetEnumerator())
                    {
                        ...
                        {
                            var seqStmt = MkZingSeq(
                                            MkZingAssignOrCast(MkZingDot("myHandle", "currentEvent"), PType.Event, MkZingIdentifier("null"), PType.Nil),
                                            MkZingAssignOrCast(MkZingDot("myHandle", "currentArg"), PType.Any, MkZingIdentifier("null"), PType.Nil),
                                            ctxt.translationContext == TranslationContext.Entry ? Factory.Instance.AddArg(ZingData.App_Goto, Factory.Instance.MkCnst("transition_" + entityName)) : Factory.Instance.AddArg(ZingData.App_Return, ZingData.Cnst_True));
                            // No user defined code here, so don't need to emitSideffects
                            return new Tuple<AST<Node>, TypeCheckInfo>(seqStmt, new TypeCheckInfo(new PNilType()));
                        }
                    }
                }
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    if (it.Current.Item2.type.isSubtypeOf(returnType))
                    {
                        ...
                        {
                            return new Tuple<AST<Node>, TypeCheckInfo>(AddArgs(ZingData.App_Return, ZingData.Cnst_Nil), new TypeCheckInfo(new PNilType(), it.Current.Item2.isGhost));
                        }
                        else
                        {
                            if (it.Current.Item2.type != returnType)
                            {
                                retNode = ctxt.getTmpVar(returnType, "tmpRetVar");
                                ctxt.addSideEffect(MkZingAssignOrCast(retNode, returnType, it.Current.Item1, it.Current.Item2.type));
                            }
                            else
                            {
                                retNode = it.Current.Item1;
                            }

                            return new Tuple<AST<Node>, TypeCheckInfo>(AddArgs(ZingData.App_Return, retNode), new TypeCheckInfo(new PNilType(), it.Current.Item2.isGhost));
                        }
                    }
                    else
                    {
            ...
            else if (funName == PData.Con_Scall.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    var save = MkZingSeq(MkZingAssign(savedDeferredSet, stackDeferredSet), MkZingAssign(savedActionSet, stackActionSet));
                    var saveCurrentEvent = MkZingAssign(savedCurrentEvent, currentEvent);
                    var saveCurrentArg = MkZingAssign(savedCurrentArg, currentArg);
                    var update = MkZingSeq(MkZingAssign(stackActionSet, currentActionSet),
                                           MkZingAssign(stackDeferredSet, MkZingCall(MkZingDot("Main", "CalculateComplementOfEventSet"), currentActionSet)));
                    var updateCurrentEvent = MkZingAssign(currentEvent, MkZingIdentifier("null"));
                    var updateCurrentArg = MkZingAssign(currentArg, MkZingIdentifier("null"));
                    var push = MkZingAssign(MkZingIdentifier("localActions"), MkZingCall(MkZingDot("LocalActions", "Construct"), MkZingIdentifier("localActions")));
                    ...
                    var gotoStmt = Factory.Instance.AddArg(ZingData.App_Goto, Factory.Instance.MkCnst("transition_" + entityName));
                    ...
                    // No user defined code here, so don't need to emitSideffects
                    return new Tuple<AST<Node>, TypeCheckInfo>(MkZingSeq(save, saveCurrentEvent, saveCurrentArg, update, updateCurrentEvent, updateCurrentArg, push, callStmt, pop, traceStmt, restore, ifStmt), new TypeCheckInfo(new PNilType()));
                }
            }
            else if (funName == PData.Con_Seq.Node.Name)
            {
            ...
            {
                AST<Node> nondet = null;
                AST<FuncTerm> nondetChoice = null;
                using (var it = children.GetEnumerator())
                {
                    it.MoveNext();
                    ...
                    if (condExpr.Item1 == null)
                    {
                        nondet = MkZingIdentifier("nondet");
                        nondetChoice = MkZingCall(Factory.Instance.MkCnst("choose"), Factory.Instance.MkCnst("bool"));
                        result = Factory.Instance.AddArg(ZingData.App_ITE, nondet);
                    }
                    else
                    {
                        result = Factory.Instance.AddArg(ZingData.App_ITE, condExpr.Item1);
                    }

                    // Order else,then is the reverse of the order in which the side effect stacks were pushed(then, else).
                    var cookedElse = ctxt.emitZingSideEffects(zingWrapExprToStmts(elseStmt.Item1));
                    ...
                    outTerm = new Tuple<AST<Node>, TypeCheckInfo>(result, new TypeCheckInfo(new PNilType(), condExpr.Item2.isGhost));
                }
            }
            else if (funName == PData.Con_Payload.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    while (it.MoveNext())
                    {
                        ...
                        if (lhsInfo.type == rhsInfo.type)
                            inits = AddArgs(ZingData.App_Args, lhs.Item1, AddArgs(ZingData.App_Args, Factory.Instance.ToAST(rhs), inits));
                        else
                        {
                            var tmpCastVar = ctxt.getTmpVar(lhsInfo.type, "tmpCastVar");
                            ctxt.addSideEffect(MkZingAssignOrCast(tmpCastVar, lhsInfo.type, Factory.Instance.ToAST(rhs), rhsInfo.type));
            ...
            else if (funName == PData.Con_Raise.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    var payloadEventMatchAssert = MkZingSeq(
                        ...
                        MkZingAssignOrCast(tmpEq, PType.Bool, MkZingCall(MkZingDot(MkZingDot("myHandle", "currentArg"), "CanCastTo"), tmpEvPayload), PType.Bool),
                        MkZingAssert(tmpEq));
                    var gotoStmt = Factory.Instance.AddArg(ZingData.App_Goto, Factory.Instance.MkCnst(string.Format("transition_{0}", entityName)));
                    var returnStmt = Factory.Instance.AddArg(ZingData.App_Return, ZingData.Cnst_False);

                    var outNode = MkZingSeq(assertStmt, traceStmt, currentEventAsgnStmt, payloadEventMatchAssert, ctxt.translationContext == TranslationContext.Entry ? gotoStmt : returnStmt);
            ...
            else if (funName == PData.Con_Call.Node.Name)
            {
                ...
                if (calleeInfo.returnType is PNilType)
                    return new Tuple<AST<Node>, TypeCheckInfo>(callExpr, new TypeCheckInfo(calleeInfo.returnType, isGhost));
                else if (calleeInfo.returnType is PPrimitiveType)
                    return new Tuple<AST<Node>, TypeCheckInfo>(callExpr, new TypeCheckInfo(calleeInfo.returnType, isGhost));
                else
                {
                    var tmpVar = ctxt.getTmpVar(calleeInfo.returnType, "retTmpVar");
                    ctxt.addSideEffect(MkZingAssign(tmpVar, callExpr));

                    return new Tuple<AST<Node>, TypeCheckInfo>(tmpVar, new TypeCheckInfo(calleeInfo.returnType, isGhost));
                }
            }
            else if (funName == PData.Con_Send.Node.Name)
            {
                using (var it = children.GetEnumerator())
                {
                    ...
                    var targetExpr = MkZingApply(ZingData.Cnst_Dot, it.Current.Item1, MkZingIdentifier("EnqueueEvent"));
                    it.MoveNext();
                    if (it.Current.Item2 == null)
                    {
                        return new Tuple<AST<Node>, TypeCheckInfo>(null, null);
                    ...
                    var callStmt = MkZingCallStmt(MkZingCall(targetExpr, eventExpr, tmpVar, Factory.Instance.MkCnst("myHandle")));
                    return new Tuple<AST<Node>, TypeCheckInfo>(callStmt, new TypeCheckInfo(new PNilType(), targetIsGhost || argIsGhost));
                }
            }
            else if (funName == PData.Con_While.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    if (condExpr.Item1 == null)
                    {
                        nondet = MkZingIdentifier("nondet");
                        nondetChoice = MkZingCall(Factory.Instance.MkCnst("choose"), Factory.Instance.MkCnst("bool"));
                        result = Factory.Instance.AddArg(ZingData.App_While, nondet);
                    }
                    else
                    {
                        result = Factory.Instance.AddArg(ZingData.App_While, condExpr.Item1);
                    }

                    if (nondet != null)
                        body = MkZingSeq(body, MkZingAssign(nondet, nondetChoice));
                    result = Factory.Instance.AddArg(result, body);
        ...
        private AST<Node> MkZingBody(Node n, string machine, TranslationContext ctxType, string entity, List<AST<Node>> locals)
        {
            ...
            Debug.Assert(ctxt.sideEffectsStack.Count == 1);
            var body = ctxt.emitZingSideEffects(zingWrapExprToStmts(tuple.Item1));
            ctxt.pushSideEffectStack();
        ...
        private AST<Node> GenerateFunMethod(string funName, FunInfo funInfo)
        {
            ...
            var machineAlias = (Id)GetArgByIndex(funInfo.funDecl, 1);
            var machineName = GetName(modelAliases[machineAlias.Name].Node, 0);
            AST<Node> funBody = MkZingBody(entry.Node, machineName, TranslationContext.Function, funName, locals);

            funBody = AddArgs(ZingData.App_LabelStmt, Factory.Instance.MkCnst("dummy"), funBody);
        ...
        private void MkZingClasses(List<AST<Node>> elements)
        {
            foreach (string machineName in allMachines.Keys)
            {
                ...
                foreach (var funName in allMachines[machineName].funNameToFunInfo.Keys)
                {
                    ...
                    methods = AddArgs(ZingData.App_MethodDecls, GenerateFunMethod(funName, funInfo), methods);
                }
        ...
        {
            Id machineAlias = (Id)GetArgByIndex(ft, index);
            FuncTerm machineDecl = modelAliases[machineAlias.Name].Node;
        ...
        private string getOwnerFromNode(Node n)
        {
            return ((Cnst)GetArgByIndex(modelAliases[((Id)n).Name].Node, 0)).GetStringValue();
        }
        ...
        private IEnumerable<Node> Model_UnFold(GenericCtxt ctxt, Node n)
        {
            if (n.NodeKind == NodeKind.ModelFact)
            {
                ...
                {
                    ctxt.curMachine = lookupAliasName(GetArgByIndex((FuncTerm)inner, 1));
                    unsetMachine = true;
                }
                ...
                {
                    var stateNode = lookupAlias(GetArgByIndex((FuncTerm)inner, 0));
                    ctxt.curMachine = lookupAliasName(GetArgByIndex(stateNode.Node, 1));
                    ctxt.curState = ((Cnst)GetArgByIndex(stateNode.Node, 0)).GetStringValue();
                    unsetMachine = true;
                    unsetState = true;
                }
                else if (isInstanceOf(inner, PData.Con_VarDecl))
                {
                    ctxt.curMachine = getOwnerFromNode(GetArgByIndex((FuncTerm)inner, 1));
                    unsetMachine = true;
                }
                else if (isInstanceOf(inner, PData.Con_TransDecl))
                {
                    var stateNode = modelAliases[((Id)GetArgByIndex((FuncTerm)inner, 0)).Name];
                    ctxt.curMachine = getOwnerFromNode(GetArgByIndex(stateNode.Node, 1));
                    ctxt.curState = ((Cnst)GetArgByIndex(stateNode.Node, 0)).GetStringValue();
                    unsetMachine = true;
                    unsetState = true;
                }
                ...
                {
                    ctxt.curMachine = getOwnerFromNode(GetArgByIndex((FuncTerm)inner, 0));
                    unsetMachine = true;
                }
                else if (isInstanceOf(inner, PData.Con_InStateSet))
                {
                    var state = lookupAlias(GetArgByIndex((FuncTerm)inner, 1));
                    ctxt.curMachine = lookupAliasName(GetArgByIndex((FuncTerm)state.Node, 1));
                    unsetMachine = true;
                }
        ...
        private AST<FuncTerm> lookupAlias(Node n)
        {
            return modelAliases[((Id)n).Name];
        }

        private string lookupAliasName(Node n)
        {
            return ((Cnst)GetArgByIndex(lookupAlias(n).Node, 0)).GetStringValue();
        }
        ...
        private int ComputePayloadSets(GenericCtxt ctxt, Node n, IEnumerable<int> dummy)
        {
            if (isInstanceOf(n, PData.Con_Scall))
            {
                ...
                // Normal transition
                var dst = ((Cnst)GetArgByIndex(modelAliases[((Id)GetArgByIndex((FuncTerm)n, 2)).Name].Node, 0)).GetStringValue();
                ...
                string state = lookupAliasName(GetArgByIndex((FuncTerm)n, 1));
            ...
            else if (isInstanceOf(n, PData.Con_Install))
            {
                ...
                string eventName;
                GetEventInfo(GetArgByIndex((FuncTerm)n, 1), out eventName, out eventArgTypeName);
                string action = ((Cnst)GetArgByIndex(modelAliases[((Id)GetArgByIndex((FuncTerm)n, 2)).Name].Node, 0)).GetStringValue();
        ...
        private int ComputeSubmachines(GenericCtxt ctxt, Node n, IEnumerable<int> dummy)
        {
            ...
            {
                var setName = lookupAliasName(GetArgByIndex((FuncTerm)n, 0));
                var stateName = lookupAliasName(GetArgByIndex((FuncTerm)n, 1));
        ...
        private int CheckSubmachineTransitions(GenericCtxt ctxt, Node n, IEnumerable<int> dummy)
        {
            ...
            {
                var from = lookupAliasName(GetArgByIndex((FuncTerm)n, 0));
                var to = lookupAliasName(GetArgByIndex((FuncTerm)n, 2));
        ...
        private bool GenerateMachineData(AST<Model> model)
        {
            ...
            foreach (var term in terms)
            {
                try
                {
                    using (var it = term.Node.Args.GetEnumerator())
                    {
                        ...
                        it.MoveNext();
                        var machineAlias = ((Id)it.Current).Name;
                        var machineDecl = modelAliases[machineAlias].Node;
                        var machineName = GetName(machineDecl, 0);
            ...
            foreach (var term in terms)
            {
                try
                {
                    using (var it = term.Node.Args.GetEnumerator())
                    {
                        ...
                        it.MoveNext();
                        var targetStateDecl = modelAliases[((Id)it.Current).Name].Node;
            ...
            foreach (var term in terms)
            {
                try
                {
                    using (var it = term.Node.Args.GetEnumerator())
                    {
                        ...
                        it.MoveNext();
                        var actionOwnerMachineDecl = modelAliases[((Id)it.Current).Name].Node;
            ...
            foreach (var term in terms)
            {
                try
                {
                    using (var it = term.Node.Args.GetEnumerator())
                    {
                        ...
                        GetEventInfo(it.Current, out eventName, out eventArgTypeName);
                        it.MoveNext();
                        var actionDecl = modelAliases[((Id)it.Current).Name].Node;
            ...
            foreach (var term in terms)
            {
                try
                {
                    using (var it = term.Node.Args.GetEnumerator())
                    {
                        ...
                        {
                            it.MoveNext();
                            machineInfo.initStateDecl = modelAliases[((Id)it.Current).Name].Node;
            ...
            foreach (var term in terms)
            {
                try
                {
                    using (var it = term.Node.Args.GetEnumerator())
                    {
                        it.MoveNext();
                        string eventSetName, machineName;
                        DeconstructEventSetDecl(modelAliases[((Id)it.Current).Name].Node, out eventSetName, out machineName);
                        it.MoveNext();
                        var eventName = GetName(modelAliases[((Id)it.Current).Name].Node, 0);
            ...
            foreach (var term in terms)
            {
                ...
                {
                    it.MoveNext();
                    var funAlias = ((Id)it.Current).Name;
                    var funDecl = modelAliases[funAlias].Node;
        ...
        void DeconstructEventSetDecl(FuncTerm term, out string eventSetName, out string machineName)
        {
            using (var enumerator = term.Args.GetEnumerator())
            {
                ...
                enumerator.MoveNext();
                var machine = (eventSetName == "None") ? (FuncTerm)enumerator.Current : modelAliases[((Id)enumerator.Current).Name].Node;
