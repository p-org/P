    ...
    class PToZing
    {
        ...
        private AST<Node> MkZingInvokeWrapperFun(string machineName, string entityName, TranslationContext type)
        {
            ...
            body.Add(MkZingIf(MkZingEq(MkZingDot("cont", "reason"), MkZingDot("ContinuationReason", "Leave")),
                MkZingSeq(
                type == TranslationContext.Action ? Compiler.AddArgs(ZingData.App_Goto, Factory.Instance.MkCnst("exit")) : type == TranslationContext.Exit ? (AST<Node>)ZingData.Cnst_Nil : Compiler.AddArgs(ZingData.App_Goto, Factory.Instance.MkCnst("wait_" + entityName)))));
            body.Add(MkZingIf(MkZingEq(MkZingDot("cont", "reason"), MkZingDot("ContinuationReason", "Return")),
                MkZingSeq(
                    ...
                    type == TranslationContext.Action ? Compiler.AddArgs(ZingData.App_Return, ZingData.Cnst_True) : Compiler.AddArgs(ZingData.App_Return, ZingData.Cnst_Nil))));
            body.Add(MkZingIf(MkZingEq(MkZingDot("cont", "reason"), MkZingDot("ContinuationReason", "Delete")),
                MkZingSeq(
                    ...
                    MkZingAssignOrCast(currentArg, PType.Any, MkZingIdentifier("null"), PType.Nil),
                    type == TranslationContext.Action ? Compiler.AddArgs(ZingData.App_Return, ZingData.Cnst_True) : Compiler.AddArgs(ZingData.App_Goto, Factory.Instance.MkCnst("transition_" + entityName)))));
            body.Add(MkZingIf(MkZingEq(MkZingDot("cont", "reason"), MkZingDot("ContinuationReason", "Raise")),
                MkZingSeq(
                    ...
                    type == TranslationContext.Action ? Compiler.AddArgs(ZingData.App_Return, ZingData.Cnst_False) : Compiler.AddArgs(ZingData.App_Goto, Factory.Instance.MkCnst("transition_" + entityName)))));
        ...
        private AST<Node> GenerateRunHelperMethodDecl(string machineName)
        {
            ...
            foreach (var stateName in compiler.allMachines[machineName].stateNameToStateInfo.Keys)
            {
                ...
                foreach (var eventName in callTransitions.Keys)
                {
                    ...
                    callTransitionStmt = Compiler.AddArgs(ZingData.App_ITE, condExpr, MkZingSeq(save, update, push, callStmt, pop, restore, ite), callTransitionStmt);
                }
        ...
        private ZingTranslationInfo ZingEntryFun_Fold_Impl(ZingEntryFun_FoldContext ctxt, Node n, IEnumerable<ZingTranslationInfo> children)
        {
            ...
            if (funName == PData.Con_Assert.Node.Name)
            {
                using (var it = children.GetEnumerator())
                {
                    ...
                    return new ZingTranslationInfo(Compiler.AddArgs(ZingData.App_Assert, it.Current.node), new PNilType(), it.Current.isGhost);
                }
            }
            else if (funName == PData.Con_DataOp.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    {
                        compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Cannot index a real sequence or map with a ghost expressions."), 0, compiler.CompilingProgram));
                        return null;
                    }
                    ...
                    if (!isGhost && it.Current.isGhost)
                    {
                        compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Cannot insert a ghost expression into a real sequence or map."), 0, compiler.CompilingProgram));
                        return null;
                    }
            ...
            else if (funName == PData.Con_Assign.Node.Name)
            {
                using (var it = children.GetEnumerator())
                {
                    ...
                    else
                    {
                        ...
                        {
                            compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Cannot assign ghost expression to real variable."), 0, compiler.CompilingProgram));
                            return null;
                        }
            ...
            else if (funName == PData.Con_ITE.Node.Name)
            {
                using (var it = children.GetEnumerator())
                {
                    ...
                    if (!inGhostContext)
                    {
                        ...
                        {
                            compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Use of nondet in a real machine is not allowed."), 0, compiler.CompilingProgram));
                            return null;
                        }
                        if (condExpr.isGhost && !compiler.allMachines[machineName].isGhost)
                        {
                            compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Use of ghost expression in a conditional in a real machine is not allowed."), 0, compiler.CompilingProgram));
                            return null;
                        }
                    }
            ...
            else if (funName == PData.Con_Use.Node.Name)
            {
                ...
                if (kind.Name == PData.Cnst_Var.Node.Name)
                {
                    ...
                    {
                        compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Use of undeclared local variable {0}.", varName), 0, compiler.CompilingProgram));
                        return null;
                    }
                    ...
                    else if (arity == 2)
                    {
                        ...
                        else if (pOp.Name == PData.Cnst_Idx.Node.Name)
                        {
                            ...
                            if (arg1.type is PTupleType)
                            {
                                ...
                                return new ZingTranslationInfo(MkZingDot(arg1.node, Compiler.getTupleField(elInd)), baseType.elements.ElementAt(elInd), arg1.isGhost);
                            }
            ...
            else if (funName == Compiler.Con_LabeledExpr)
            {
                using (var it = children.GetEnumerator())
                {
                    ...
                    return new ZingTranslationInfo(Compiler.AddArgs(Compiler.App_LabeledExpr, Factory.Instance.ToAST(Compiler.GetArgByIndex(ft, 0)), it.Current.node), it.Current.type, it.Current.isGhost, it.Current.stateName, it.Current.isMapIndex, it.Current.isKeys);
                }
            }
            else if (funName == PData.Con_New.Node.Name)
            {
                using (var it = children.GetEnumerator())
                {
                    ...
                    return new ZingTranslationInfo(MkZingCreateMachineCall(ctxt, typeName, inits.Node), new PIdType(), compiler.allMachines[typeName].isGhost, true, afterLabel);
            ...
            else if (funName == PData.Con_Raise.Node.Name)
            {
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    {
                        compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("The payload of a raise must be real."), 0, compiler.CompilingProgram));
                        return null;
                    }
            ...
            else if (funName == PData.Con_Call.Node.Name)
            {
                ...
                foreach (var child in children)
                {
                    ...
                    {
                        compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Cannot pass ghost expression to real parameter."), 0, compiler.CompilingProgram));
                        return null;
                    }
                ...
                AST<Node> callStmt = MkZingSeq(
                    ...
                    Compiler.AddArgs(ZingData.App_ITE, MkZingEq(MkZingDot("entryCtxt", "reason"), MkZingDot("ContinuationReason", "ReturnVal")),
                        processOutput,
                        MkZingIf(MkZingNeq(MkZingDot("entryCtxt", "reason"), MkZingDot("ContinuationReason", "Return")),
                            MkZingSeq(
                                MkZingCallStmt(MkZingCall(MkZingDot("entryCtxt", "PushReturnTo"), Factory.Instance.MkCnst(ctxt.labelToId(beforeLabel)))),
                                MkZingReturn(MkZingIdentifier("entryCtxt"))))));
                ...
                using (var it = children.GetEnumerator())
                {
                    ...
                    if (it.Current.node != ZingData.Cnst_Nil)
                    {
                        if (it.Current.isGhost)
                        {
                            if (!targetIsGhost)
                            {
                                compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("The payload of a send to a real machine must also be real."), 0, compiler.CompilingProgram));
                                return null;
                            }
                            if (payloadType == PType.Id)
                            {
                                compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Ghost payload of type machine id is not allowed."), 0, compiler.CompilingProgram));
                                return null;
                            }
                        }
                    }
            ...
            else if (funName == PData.Con_While.Node.Name)
            {
                using (var it = children.GetEnumerator())
                {
                    ...
                    if (!compiler.allMachines[machineName].isGhost)
                    {
                        if (condExpr == null)
                        {
                            compiler.errors.Add(new Flag(SeverityKind.Error, n, string.Format("Use of nondet in a real machine is not allowed."), 0, compiler.CompilingProgram));
        ...
        private AST<FuncTerm> MkZingTupleClassDefinition(IEnumerable<Tuple<PType, string, string>> fieldDesc, AST<Node> zType, string zTypeName, PType pType)
        {
            ...
            foreach (var field in fieldDesc)
            {
                ...
                equalsBody.Add(equalsCtxt.emitZingSideEffects(Compiler.AddArgs(ZingData.App_ITE, check, Compiler.AddArgs(ZingData.App_Return, ZingData.Cnst_False), ZingData.Cnst_Nil)));
            ...
            foreach (var subT in compiler.subtypes[pType])
            {
                ...
                for (int i = 0; i < fieldDesc.Count(); i++)
                {
                    ...
                    else
                    {
                        ...
                        checks.Add(ctxt.emitZingSideEffects(Compiler.AddArgs(ZingData.App_ITE, cond, Compiler.AddArgs(ZingData.App_Return, ZingData.Cnst_False), ZingData.Cnst_Nil)));
                        ctxt.pushSideEffectStack();
                    }
                }
