...
namespace Plang.Compiler.Backend.Containment
{
    internal class CompilationContext : CompilationContextBase
    {
        ...
        IDictionary<PEvent, int> eventToIds;
        IDictionary<State, int> stateToIds;
            ...
            switch (decl) {
                ...
                case State state:
                    if (!stateToIds.ContainsKey(state)) {
                        int newId = stateToIds.Count;
                        stateToIds.Add(state, newId);
                    }
                    return $"{stateToIds[state]}";
                case PEvent pEvent:
                    if (!eventToIds.ContainsKey(pEvent)) {
                        int newId = eventToIds.Count;
                        eventToIds.Add(pEvent, newId);
                    }
                    return $"{eventToIds[pEvent]}";
                default:
