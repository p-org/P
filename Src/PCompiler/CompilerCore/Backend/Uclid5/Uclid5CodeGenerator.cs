...
public class Uclid5CodeGenerator : ICodeGenerator
{
    ...
    private void GenerateMain(Machine machine, State state, PEvent @event, List<Invariant> goals, List<Invariant> requires, bool generateSanityChecks)
    {
        ...
        GenerateMachineProcedures(machines, goals, generateSanityChecks); // generate machine methods, called by handlers below
        EmitLine("");
    ...
    private void GenerateMachineProcedures(List<Machine> machines, List<Invariant> goals, bool generateSanityChecks = false)
    {
        foreach (var m in machines)
        {
            ...
            foreach (var b in bound)
            {
                switch (b.Type)
                {
                    case PermissionType {Origin: Machine} pt:
                        bounds.Add(ExprToString(new TestExpr(b.SourceLocation, new VariableAccessExpr(b.SourceLocation, b), pt.Origin)));
                        break;
                    case PermissionType {Origin: Interface} pt:
                        ...
                        {
                            bounds.Add(ExprToString(new TestExpr(b.SourceLocation, new VariableAccessExpr(b.SourceLocation, b), m)));
                        }
