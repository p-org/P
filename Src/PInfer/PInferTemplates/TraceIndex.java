public class TraceIndex {
    Map<List<PEvent<?>>, Map<Class<?>, List<Integer>>> indexedTraces;
    ...
    public Map<Class<?>, List<Integer>> getTraceIndex(List<PEvent<?>> trace) {
        buildIndex(trace);
        return indexedTraces.get(trace);
    }

    public List<Integer> getIndices(List<PEvent<?>> trace, Class<?> cls) {
        buildIndex(trace);
        return indexedTraces.get(trace).getOrDefault(cls, EMPTY);
    }

    public boolean indexed(List<PEvent<?>> trace, Class<?> cls) {
        return !getIndices(trace, cls).isEmpty();
    }

    public void buildIndex(List<PEvent<?>> trace) {
