...
public class ExplicitSymmetryTracker extends SymmetryTracker {
  ...
  public SymmetryTracker getCopy() {
    return new ExplicitSymmetryTracker(typeToSymmetryClasses, typeToPendingMerges);
  }
      ...
      for (GuardedValue<Machine> lhsRepGV : lhsRepGVs) {
        ...
        for (GuardedValue<Machine> rhsRepGV : rhsRepGVs) {
          ...
          if (lhsRepMachine == rhsRepMachine) {
            ...
            if (getSymEquivGuard(lhsRepMachine, rhsRepMachine)) {
              symEqGuard = Guard.constTrue();
            }
          }
        }
      }
  ...
  private boolean getSymEquivGuard(Machine m1, Machine m2) {
    assert (m1 != m2);
