    ...
    public static PrimitiveVS<String> formattedStringVS(Guard pc, String baseString, ValueSummary... args) {
        ...
        if (args.length == 0) {
            ...
            if (baseString.contains(" ")) {
                ...
                List<GuardedValue<List<Object>>> concreteArgs = getConcreteValues(pc, x -> false, Concretizer::concretize, args);
                for (int i = 0; i < concreteArgs.size(); i++) {
                    ...
                    final String mapped = messageFormatter(baseString, guardedArgsValues.toArray(new Object[guardedArgsValues.size()]));
