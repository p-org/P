    ...
    on eNext do {
        ...
        while (j < numQuora) {
          if (forallCond[j]) {
            ...
            if (sizeof(Q1bVal[j][ballot]) == 0) {
              ...
              k = 0;
              while (k < sizeof(Q1bVal[j][ballot])) {
                val = Q1bVal[j][ballot][k];
                if (!(val in M2a[ballot])) {
                   // m.mval = v
                  choices += (sizeof(choices), (1, ballot, val));
                }
                k = k + 1;
              }
            }
      ...
      while (i < NumAcceptors) {
        ...
        while (j < NumBallots) {
          ...
          // Phase2b
          if (0 in M2a[j] && !(0 in M2b[i][j])) {
            choices += (sizeof(choices), (3, i, j));
          }
          if (1 in M2a[j] && !(1 in M2b[i][j])) {
      ...
      if (sizeof(choices) > 0) {
        ...
        if (pcChoice >= 3) {
          ...
          raise Phase2b, (arg, choice.2, val);
        }
      }
    }
