...
namespace Microsoft.Pc.TypeChecker
{
    public class DefaultTranslationHandler : ITranslationErrorHandler
    {
        ...
        public DefaultTranslationHandler(ParseTreeProperty<FileInfo> originalFiles)
        {
        ...
        {
            return IssueError(location, $"enum element {location.name.GetText()} in {pEnum.Name} duplicates previous value");
        }
        ...
        public Exception DuplicateStartState(
            ...
            Machine machine)
        {
            return IssueError(location, $"state '{duplicateStart.Name}' tries to replace start state '{originalStart.Name}' in machine '{machine.Name}'");
        }
        ...
        {
            return IssueError(location, $"exit handler in state '{state.Name}' duplicates exit handler at {GetLocation(existingHandler.SourceNode)}");
        }

        public Exception DuplicateStateEntry(ParserRuleContext location, Function existingHandler, State state)
        {
            return IssueError(location, $"entry handler in state '{state.Name}' duplicates entry handler at {GetLocation(existingHandler.SourceNode)}");
        }
        ...
        {
            return IssueError(location, $"function or constructor call expected {expectedCount} arguments, got {actualCount}");
        }
        ...
        {
            return IssueError(location, $"got type: {actual.OriginalRepresentation}, expected: {string.Join(", ", expected.Select(e => e.Name))}");
        }
        ...
        public Exception BinOpTypeMismatch(PParser.BinExprContext location, PLanguageType lhsType, PLanguageType rhsType)
        {
            return IssueError(location,
                              location.op,
