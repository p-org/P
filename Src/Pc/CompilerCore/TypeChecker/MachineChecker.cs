...
namespace Plang.Compiler.TypeChecker
{
    public static class MachineChecker
    {
            ...
            Function targetFunction)
        {
            ...
            if (entrySignature.Count == 0) return;

            if (entrySignature.Count == 1 && entrySignature[0].IsAssignableFrom(eventPayloadType)) return;
        ...
        {
            if (!(startState.Entry?.Signature.Parameters.Count > 0)) return PrimitiveType.Null;
        ...
        private static State FindStartState(Machine machine, ITranslationErrorHandler handler)
        {
            ...
            if (!foundStartState || machine.StartState == null) throw handler.MissingStartState(machine);
