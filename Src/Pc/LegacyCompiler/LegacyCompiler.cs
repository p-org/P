namespace Microsoft.Pc
{
    ...
    public class LegacyCompiler : ICompiler
    {
        ...
        public LegacyCompiler(bool shortFileNames)
        {
        ...
        private void PrintResult(InstallResult result)
        {
            ...
            {
                if (f.Item2.Severity != Formula.API.SeverityKind.Error)
                {
                    continue;
                }
                Log.WriteMessage(
        ...
        void AddCompilerErrorFlag(FuncTerm ft, ref int compileErrorCount)
        {
            ...
            if (ftName == "ERROR0")
            {
                errorReporter.AddFlag(new Flag(Formula.API.SeverityKind.Error, errorSpan1, errorMsg, 1, errorSpan1.Program));
            }
