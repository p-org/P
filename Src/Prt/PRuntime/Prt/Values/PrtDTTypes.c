	...
	_In_ PRT_FORGN_ISEQUAL     eqTester)
{
	...
	prtType.typeUnion.forgn->freer = freer;
	prtType.typeUnion.forgn->hasher = hasher;
	prtType.typeUnion.forgn->eqTester = eqTester;
...
PRT_TYPE PrtMkNmdTupType(_In_ PRT_UINT32 arity)
{
	...
	prtType.typeUnion.nmTuple->fieldTypes = (PRT_TYPE *)PrtCalloc((size_t)arity, sizeof(PRT_TYPE));
	return prtType;
}
...
PRT_TYPE PrtMkAbsentType()
{
	...
	abstType.typeUnion.forgn->typeTag.data3 = 0;
	abstType.typeUnion.forgn->typeTag.data4 = 0;
	abstType.typeUnion.forgn->cloner = &PrtAbsentTypeClone;
	abstType.typeUnion.forgn->freer = &PrtAbsentTypeFree;
	abstType.typeUnion.forgn->hasher = &PrtAbsentTypeGetHashCode;
	abstType.typeUnion.forgn->eqTester = &PrtAbsentTypeIsEqual;
