...
foreach($n in $xdox.configuration.AllNodes.ChildNodes)
{
     ...
     $command = $psExec + " -t -u planguser -p Pldi2015 \\$nn PrtDistService.exe"
     Invoke-Expression $command
     Write-host -ForegroundColor Green "Killed PrtDistservice on $nn"
}

[string]$centralserver = $xdox.configuration.CentralServer
$command = $psExec + " -t -u planguser -p Pldi2015 \\$nn PrtDistCentralServer.exe"
Invoke-Expression $command
Write-host -ForegroundColor Green "Killed PrtDistCentralServer on $nn"
