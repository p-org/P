...
static void LogHandler(PRT_STEP step, PRT_MACHINEINST *context, PRT_MACHINEINST *sender, PRT_VALUE* eventId, PRT_VALUE* payload)
{
    steps++;
    ...
    if (!stopping && now - startTime > runTime)
    {
		...
		PrtSend(context, context, haltEvent, nullValue, PRT_FALSE);
