...
main machine TheGodMachine {
	...
	start state Init {
		entry {
			...
			servers.two = new ChainReplicationServer((isHead = false, isTail = true, smId = 2));
			servers.one = new ChainReplicationServer((isHead = true, isTail = false, smId = 1));
			
			send(servers.two, predSucc, (pred = servers.one, succ = servers.two));
			send(servers.one, predSucc, (pred = servers.one, succ = servers.two));
			//create the client and start the game
			new Client((head = servers.one, tail = servers.two, startIn = 1));
			new Client((head = servers.one, tail = servers.two, startIn = 100));
			raise(delete);
		}
	}
