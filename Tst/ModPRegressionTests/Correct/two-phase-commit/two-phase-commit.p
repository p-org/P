...
machine Replica {
	...
	state Loop {
		...
		on GLOBAL_COMMIT do (payload:int) {
			assert (pendingWriteReq.seqNum >= payload);
			if (pendingWriteReq.seqNum == payload) {
				data_v[pendingWriteReq.idx] = pendingWriteReq.val;
				lastSeqNum = payload;
			}
		}
...
machine Coordinator {
	...
	fun DoRead(payload: (client:machine, idx:int)) {
		if (payload.idx in data_v) {
			announce announce_READ_SUCCESS, (idx=payload.idx, val=data_v[payload.idx]);
			send payload.client, READ_SUCCESS, data_v[payload.idx];
		} else {
...
spec M observes announce_WRITE, announce_READ_SUCCESS, announce_READ_UNAVAILABLE {
	...
	start state Init {
		on announce_WRITE do (payload: (idx:int, val:int)) { data_v[payload.idx] = payload.val; }
		on announce_READ_SUCCESS do (payload : (idx:int, val:int)) { 
			assert(payload.idx in data_v);
			assert(data_v[payload.idx] == payload.val);
		}
		on announce_READ_UNAVAILABLE do (payload: int) {
			assert(!(payload in data_v));
		}
	}
}
