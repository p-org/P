  ...
  start state Init {
    ...
    on eReplyP1 do (pld : (acceptor : int, b : int)) {
      ...
      while (i < sizeof(hVal[pld.acceptor])) {
        SentP1B[pld.b] += (hVal[pld.acceptor][i]); //(sizeof(SentP1B[pld.b]), hVal[pld.acceptor]);
    ...
    on eCollectP1 do (leader : int) {
      ...
      if (highestP1ABallot <= b[leader]) {
        ...
        while (i < sizeof(SentP1B[b[leader]])) {
          pVal[leader] += (SentP1B[b[leader]][i]); //(sizeof(pVal[leader]), SentP1B[b[leader]][i]);
          i = i + 1;
        }
      }
      pc[leader] = 1;
    ...
    on eSendP2 do (leader : int) {
      ...
      if (sizeof(pVals) == 0) {
        SentP2A[b[leader]] += ((s[leader], b[leader], leader));
      } else {
    ...
    on eCollectP2 do (leader : int) {
      ...
      if (elected[leader]) {
        // SendP3
        SentP3A[b[leader]] += ((slot=s[leader], dcd=lv[leader]));
        s[leader] = s[leader] + 1;
      }
      pc[leader] = 0;
      send driver, eNext;
