...
namespace CreatePtFolders
{
	class Program
	{
		static void Main(string[] args)
		{
            ...
            string testDirName = "C:\\VanillaPLanguage\\P\\Tst\\RegressionTests\\Liveness";
            ...
            //Temp code: checking tests where \Pt folder is missing:
            //foreach (var cand in allFolders)
            //{
            //    string[] names = cand.Split(Path.DirectorySeparatorChar);
            //    var bottomFolder = names.Last();
            //    if (bottomFolder == "Zing" || bottomFolder == "Zing_disabled")
            //    {
            //        //Console.WriteLine("found Zing* folder {0}", cand);
            //        //Checking that the corresp. Pt folder exists:
            ...
            //        {
            //            Console.WriteLine("Pt folder does not exists for {0}", ptDirPath);
            //        }
            //    }
            //}
            ...
            foreach (var cand in allFolders)
            {
                ...
                if (bottomFolder == "Prt" || bottomFolder == "prt")
                {
                    ...
                    else
                    {
                        ...
                        //If Zing folder does not exists, nothing to compare:
                        if (!(Directory.Exists(Path.Combine(folderInfo.Parent.FullName, "Zing"))) ||
                            !(Directory.Exists(Path.Combine(folderInfo.Parent.FullName, "zing"))))
                        {
                            Console.WriteLine("Zing folder does not exist; look manually under {0}", cand);
                            result = -1;
                        }
                        else
                        {
                                ...
                                if (zingAcceptor.Contains("Error:"))
                                {
                                    ...
                                    if (ptAcceptor.Contains("ERROR:") || ptAcceptor.Contains("EXIT: 1"))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            //  Zing acceptor example1 (in fact, the only assertion failure detecte by Zing for RegressionTests suite):
                                            //      Error:                                     --find this line
                                            //      P Assertion failed:
                                            //      Expression: assert(false)
                                            //      Comment: Unhandled event exception by machine Main
                                            //  Zing acceptor example2:
                                            //      Error:
                                            //      P Assertion failed:
                                            //      Expression: assert(false)
                                            //      Comment: key not found         --print out comment
                                            ...
                                            if (zingAcceptor.Contains("P Assertion failed:"))
                                            {
                                                ...
                                                {
                                                    int zingCommentInd = zingAcceptor.LastIndexOf("Comment");
                                                    string zingComment = zingAcceptor.Substring(zingCommentInd + 7, zingAcceptor.Length - (zingCommentInd + 7));
                                                    //zingComment could be an empty string
                                                    Console.WriteLine("zingAcceptor error message: {0}", zingComment);

                                                    int ptExceptionInd = ptAcceptor.LastIndexOf("Exception of type");
                                                    string ptException = ptAcceptor.Substring(ptExceptionInd + 18, ptAcceptor.Length - (ptExceptionInd + 18));
                                                    Console.WriteLine("ptAcceptor exception: {0}", ptException);
                                                    Console.WriteLine("Pt acceptor contains *ERROR: Exception*, success; compare error messages above manually");
                                                    result = 1;
                                                }
                                                else
                                                {
                                    ...
                                    {
                                        Console.WriteLine("error in Zing, but no error in PTester, look manually under {0}", cand);
                                        result = -1;
                                    }
                                }
                                else
                                {
                                    Console.WriteLine("Unknown case (unreachable): neither error or assert failure in Zing, look manually under {0}", cand);
                                    result = -1;
                                }
