...
event ePrepareSuccess: (transId: int);
event ePrepareFailed: (transId: int);
event eGlobalAbort: (transId: int);
event eGlobalCommit: (transId: int);
...
event eMonitor_WriteSuccess:(key:int, val:int);
event eMonitor_ReadSuccess:(key:int, val:int);
event eMonitor_ReadUnAvailanle:int;
