...
namespace ChandyMisraRacey
{
        ...
        private class Init : State
        {
            protected override void OnEntry()
            {
                ...
                if (machine.N == 4)
                {
                    ...
                    this.Send(machine.A0, new eNotify(new Tuple<Machine, Message, int>(
                        null, new Message(0), 0)));
                }
                else
                {
                    this.Send(machine.A0, new eAddNeighbour(new Tuple<Machine, int>(
                    ...
                    this.Send(machine.A4, new eNotify(new Tuple<Machine, Message, int>(
                        null, new Message(0), 0)));
                }
            }
        }
    ...
    internal class SPProcess : Machine
    {
        ...
        private void Process()
        {
            ...
            var sender = ((Tuple<Machine, Message, int>)this.Payload).Item1;
            var message = ((Tuple<Machine, Message, int>)this.Payload).Item2;
            var w = ((Tuple<Machine, Message, int>)this.Payload).Item3;
            var raceyMessage = this.Message;

            if (this.Message.Value == -1 || this.Message.Value > message.Value + w)
            {
                this.Message.Value = message.Value + w;
                this.N = sender;

                for (int idx = 0; idx < this.Neighbours.Count; idx++)
                {
                    this.Send(this.Neighbours[idx], new eNotify(new Tuple<Machine, Message, int>(
                        this, this.Message, this.NeighboursD[idx])));
                }
            }

            raceyMessage.Value = 5;
        }
