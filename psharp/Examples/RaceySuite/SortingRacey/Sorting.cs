    ...
    internal class SProcess : Machine
    {
        ...
        private class Running : State
        {
            protected override void OnEntry()
            {
                ...
                machine.Result = new Result(((Tuple<Machine, Machine, int>)this.Payload).Item3);

                this.Invoke<SortingMonitor>(new eNotifyMonitor(new Tuple<int, int>(
                    machine.Id, machine.Result.Value)));

                if (machine.Left != null)
                {
                    this.Send(machine.Left, new eNotifyLeft(machine.Result));
                }

                if (machine.Right != null)
                {
                    this.Send(machine.Right, new eNotifyRight(machine.Result));
                }
            }
        }
        ...
        private void Update()
        {
            ...
            this.Result = (Result)this.Payload;

            this.Invoke<SortingMonitor>(new eNotifyMonitor(new Tuple<int, int>(
                this.Id, this.Result.Value)));

            if (this.Left != null)
            {
                this.Send(this.Left, new eNotifyLeft(this.Result));
            }

            if (this.Right != null)
            {
                this.Send(this.Right, new eNotifyRight(this.Result));
            }
        ...
        private void ProcessLeft()
        {
            ...
            var result = (Result)this.Payload;

            if (this.Result.Value > result.Value)
            {
                this.Send(this.Right, new eUpdate(this.Result));
                this.Raise(new eUpdate(result));
            }
        }
        ...
        private void ProcessRight()
        {
            ...
            if (result.Value > this.Result.Value)
            {
                this.Send(this.Left, new eUpdate(this.Result));
                this.Raise(new eUpdate(result));
            }
        }
